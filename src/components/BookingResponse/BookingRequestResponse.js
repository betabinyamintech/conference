...
const BookingRequestResponse = ({ bookingRequestResponse, setBookingRequestResponse }) => {
    ...
    const book = async (bookingDetails) => {
        ...
        if (checkIfSubscriber == -1)
            ...
            setUserState({ ...userState, subscription: { balance: checkIfSubscriber.coinsBalance } })
            console.log("checkIfSubscriber.coinsBalance after", checkIfSubscriber.coinsBalance)
